...
namespace RemoteGitDeploy.API.Get {
    public class RepositoryHistory : IAPI {
        public string FileName => "/api/get/repository/history";
        ...
        public async Task OnRequest(HttpContext httpContext) {
            ...
            if (data.TryGetValue("id", out string id)) {
                ...
                Model.Database.RepositoryHistory[] history = await HtcPlugin.DatabaseManager.GetRepositoryHistoryAsync(repository, conn);
                if (history == null) {
